...
// Local Market is mounted too (/api/market) with its demo listings, plus Market Prices and Weather.
// MANDI_LIVE=1 also syncs today's Uttar Pradesh mandi prices from data.gov.in in the background.
// Sign in with a demo phone (9100000001 ... 9100000015, see docs/CODEBASE_STATUS.md) and PIN 246810.
...
const marketEnv = { AUTH_LOOKUP_SECRET: 'dev-only-secret-dev-only-secret-dev' };
await seedMarketDemo(pool, { env: marketEnv });
...
app.use('/api/market', createMarketRouter({ pool, auth, env: marketEnv }).router);
