...
function input(label, value, { secret = false, hint } = {}) {
  const box = el('entry');
  box.append(el('entry-label', label));
  box.append(el('entry-value', secret ? '•'.repeat(value.length) : value || '—'));
  if (hint) box.append(el('entry-hint dim', hint));
  return box;
}
function errorText(err) { return err instanceof ApiError || err?.message ? err.message : 'Network problem. Try again.'; }
...
export const AuthWelcome = {
  ...
  render() { const root = el('list'); root.append(note('Your market, your community.')); ['Sign in', 'Create account'].forEach((label, i) => root.append(el('item', `${i + 1}  ${label}`))); return root; },
  onEnter(_el, ctx, i) { draft.mode = i ? 'signup' : 'login'; draft.phone = ''; draft.pin = ''; draft.pinConfirm = ''; ctx.router.push('AuthPhone'); },
};
...
export const AuthPhone = {
  ...
  render() { const root = el('form-screen'); root.append(input('Mobile number', draft.phone, { hint: 'Digits only · 8–15 digits' })); root.append(note('Use your phone number as your account ID.')); return root; },
...
export const AuthPin = {
  ...
  render() {
    ...
    root.append(note(draft.mode === 'signup' ? 'Your PIN is private and can be changed later.' : 'Enter the PIN for this phone number.'));
    return root;
  },
...
async function advancePin(ctx) {
  ...
  else if (draft.pin !== draft.pinConfirm) { draft.pinConfirm = ''; ctx.router.replace('AuthResult', { message: 'PINs did not match. Please enter it again.', retry: 'AuthPin' }); }
...
export const ProfileRegion = {
  ...
  render(ctx) { const root = el('list'); const regions = identity.options?.regions || []; if (!regions.length) { root.append(note('Loading regions…')); options().then(ctx.rerender).catch(() => {}); return root; } regions.forEach((region, i) => root.append(el('item', `${i + 1}  ${region.name}`))); return root; },
...
async function saveSignup(ctx, index) {
  try {
    ...
    if (draft.mode === 'edit') {
      ...
      if (!result.ok || body.ok === false) throw new Error(body.error?.message || 'Could not save profile.');
...
export const Settings = {
  ...
  render() { const root = el('list'); ['My profile', 'Language', 'My crops', 'Sign out'].forEach((label, i) => root.append(el('item', `${i + 1}  ${label}`))); return root; },
...
export const ProfileSummary = {
  ...
  render() { const p = identity.profile; const root = el('form-screen'); root.append(input('Name', p?.displayName)); root.append(input('Village', p?.village)); root.append(input('Region', p?.regionName)); root.append(input('Language', p?.language)); return root; },
};
...
const LANGUAGES = [['en', 'English'], ['hi', 'हिन्दी'], ['bn', 'বাংলা'], ['vi', 'Tiếng Việt']];
