...
export const FarmGate = asyncScreen({
  ...
  renderData(data){ const root=el('list'); if(!data.items.length){ root.append(message('You have no farm yet. Start one to plan daily work.')); const r=el('item ops-action','1  Create my farm'); r.dataset.create='1'; root.append(r); return root; } data.items.forEach((f,i)=>{const r=el('item ops-team-row');r.dataset.farm=f.id;r.append(el('',`${i+1}  ${f.name}`),el('ops-task-meta',`${f.role} · ${f.open_tasks} open`));root.append(r);});return root; },
