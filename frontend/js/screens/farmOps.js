...
export const FarmGate = asyncScreen({
  ...
  renderData(data){ const root=el('list'); if(!data.items.length) root.append(message('You have no farm yet. Add one to plan daily work.'));
    data.items.forEach((f,i)=>{const r=el('item ops-team-row');r.dataset.farm=f.id;const body=el('ops-task-body');body.append(el('ops-task-title',`${i+1}  ${f.name}`),el('ops-task-meta',f.region_name||f.region_code));r.append(body,el('ops-task-meta',`${f.role} · ${f.open_tasks} open`));root.append(r);});
    const add=el('item ops-action',`${data.items.length+1}  + Add a farm`);add.dataset.create='1';root.append(add); return root; },
...
async function pickRegion(ctx,name){
  ...
  ctx.router.replace('ForumPicker',{title:'Farm region',selected:identity.profile?.regionCode,
    options:regions.map((r)=>({label:r.name,value:r.code})),onPick:(o,c)=>createFarm(c,{name,regionCode:o.value})});
}
