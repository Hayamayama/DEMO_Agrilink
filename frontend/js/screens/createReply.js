...
export default {
  ...
  render(ctx) {
    ...
    if (composer.busy) { wrap.appendChild(loadingView('Sending…')); return wrap; }
    entry?.dispose();
    entry = createTextEntry({
      max: 400, min: 2, initial: composer.text, placeholder: 'Write a helpful reply…',
      onDone: (v) => send(ctx, autoCap(v)),
    });
    wrap.appendChild(el('forum-label', 'Your reply (plain text, no links)'));
...
async function send(ctx, text) {
  if (text.length < 2) return entry.hint('Write at least 2 characters');
  composer.text = text;
  ...
  try {
    ...
    composer.busy = false;
    if (err.code === 'AUTH_REQUIRED') { expireSession(); composer.error = 'Please sign in again.'; }
